...
public class RouteSelectorScreen extends ScreenExtension implements IGui {
	...
	public RouteSelectorScreen(@Nullable Runnable onClose, ObjectImmutableList<DashboardListItem> allData, LongCollection selectedIds, Depot depot) {
		this(null, onClose, allData, selectedIds, isSingleSelect, canRepeat);
	}

	public RouteSelectorScreen(@Nullable ScreenExtension previousScreen, ObjectImmutableList<DashboardListItem> allData, LongCollection selectedIds, Depot depot) {
		this(previousScreen, null, allData, selectedIds, isSingleSelect, canRepeat);
	}

	private RouteSelectorScreen(@Nullable ScreenExtension previousScreen, @Nullable Runnable onClose, ObjectImmutableList<DashboardListItem> allData, LongCollection selectedIds, Depot depot) {
		super();
		...
		buttonDuplicateVehicleCars = new ButtonWidgetExtension(0, 0, 0, SQUARE_SIZE, TextHelper.translatable("gui.mtr.duplicate_instructions"), button -> {
			final ObjectArrayList<> existingInstructions = new ObjectArrayList<>(depot.routes);
			depot.routes.addAll(existingInstructions);
		});
	}
